...
class AddStudentDialog(QDialog):
    ...
    def create_id_field(self, layout):
        row = QHBoxLayout()
        row.addWidget(QLabel("Student ID (YYYY-NNNN):*"))
    ...
    def create_program_field(self, layout):
        ...
        row.addWidget(self.program_combo)
        layout.addLayout(row)

    def create_year_field(self, layout):
        row = QHBoxLayout()
        row.addWidget(QLabel("Year Level:*"))
    ...
    def create_gender_field(self, layout):
        ...
        row.addWidget(self.male_radio)
        row.addWidget(self.female_radio)
        layout.addLayout(row)
    ...
    def populate_form(self):
        ...
        year_index = self.year_combo.findText(str(self.student_data['year']))
        if year_index >= 0:
    ...
    def get_student_data(self):
        return {
            ...
            'program_code': self.get_selected_program_code(),
...
class AddCollegeDialog(QDialog):
    ...
    def validate_college_data(self, college_data):
        ...
        current_code = self.college_data['code'] if self.is_edit_mode else None
        valid, message = CollegeValidator.check_duplicate_code(
            self.db_manager, college_data['code'], current_code
        )
        if not valid:
    ...
    def save_college(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.is_edit_mode:
                success, message = self.db_manager.update_college(college_data, self.college_data['code'])
            else:
...
class AddProgramDialog(QDialog):
    ...
    def create_programname_field(self, layout):
        ...
        row.addWidget(self.programname_input)
        layout.addLayout(row)

    def create_programcode_field(self, layout):
        row = QHBoxLayout()
        row.addWidget(QLabel("Program Code:*"))
        self.programcode_input = QLineEdit()
        self.programcode_input.setPlaceholderText("BSCS")
        row.addWidget(self.programcode_input)
        layout.addLayout(row)

    def create_college_field(self, layout):
        row = QHBoxLayout()
        row.addWidget(QLabel("College:*"))
        self.college_combo = QComboBox()
        self.college_combo.setMinimumWidth(350)
        for college in self.colleges:
            self.college_combo.addItem(f"{college['code']} - {college['name']}", college['code'])
        row.addWidget(self.college_combo)
        layout.addLayout(row)
    ...
    def populate_form(self):
        ...
        for i in range(self.college_combo.count()):
            if self.college_combo.itemData(i) == self.program_data['college']:
    ...
    def validate_program_data(self, program_data):
        ...
        if not program_data['college']:
            QMessageBox.warning(self, "Validation Error", "Please select a college")
            self.college_combo.setFocus()
            return False

        current_code = self.program_data['code'] if self.is_edit_mode else None
        valid, message = ProgramValidator.check_duplicate_code(
            self.db_manager, program_data['code'], current_code
        )
        if not valid:
            QMessageBox.warning(self, "Duplicate Error", message)
            self.programcode_input.setFocus()
    ...
    def save_program(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.is_edit_mode:
                success, message = self.db_manager.update_program(program_data, self.program_data['code'])
            else:
...
class MainWindow(QMainWindow):
    ...
    def get_page_data(self, all_data, current_page, per_page):
        total_pages = max(math.ceil(len(all_data) / per_page), 1)
        start = (current_page - 1) * per_page
        return all_data[start:start + per_page], total_pages

    def _make_action_buttons(self, edit_cb, delete_cb):
        widget = QWidget()
        ...
        delete_btn.clicked.connect(delete_cb)
    ...
    def sort_students_by_dropdown(self, sort_by):
        ...
        if key:
            self.all_students.sort(key=lambda s: str(s.get(key, '')).lower())
            self.students_page = 1
    ...
    def delete_student(self, row):
        ...
        reply = QMessageBox.question(
            self, 'Delete Student',
            f'Are you sure you want to delete:\n\n{firstname} {lastname} (ID: {student_id})?',
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def add_action_buttons(self, row):
        widget = self._make_action_buttons(
            lambda checked, r=row: self.open_edit_student_dialog(r),
            lambda checked, r=row: self.delete_student(r)
        )
        self.dataTableStudents.setCellWidget(row, 7, widget)
    ...
    def load_students_table(self, refresh=True):
        ...
        filtered = [
            ...
            if not search_text
            or search_text in s['id'].lower()
            or search_text in s['firstname'].lower()
            or search_text in s['lastname'].lower()
            or search_text in s.get('program_name', s.get('program_code', '')).lower()
        ...
        for row, student in enumerate(students):
            ...
            table.setItem(row, 3, QTableWidgetItem(student.get('program_name', student.get('program_code', ''))))
            table.setItem(row, 4, QTableWidgetItem(student.get('college_code', '')))
            table.setItem(row, 5, QTableWidgetItem(str(student['year'])))
    ...
    def update_students_pagination(self):
        self.pageInfoLabelStudents.setText(f"Page {self.students_page} of {self.students_total_pages}")
    ...
    def load_colleges_table(self, refresh=True):
        ...
        filtered = [
            ...
            if not search_text
            or search_text in c['name'].lower()
    ...
    def add_action_buttons_college(self, row, college):
        widget = self._make_action_buttons(
            lambda checked, c=college: self.edit_college(c),
            lambda checked, c=college: self.delete_college(c)
        )
        self.dataTableColleges.setCellWidget(row, 2, widget)
    ...
    def delete_college(self, college):
        try:
            ...
            affected = [p for p in programs if p['college'] == college['code']]

            warning_msg = f"Are you sure you want to delete {college['name']} ({college['code']})?"
            if affected:
                program_list = '\n'.join([f"  • {p['name']} ({p['code']})" for p in affected[:5]])
                if len(affected) > 5:
                    program_list += f"\n  • ...and {len(affected) - 5} more"
                warning_msg += (
                    f"\n\n⚠️  WARNING: This college has {len(affected)} program(s):\n{program_list}"
                    f"\n\nThese programs will have their college set to NULL."
                )
    ...
    def update_colleges_pagination(self):
        self.pageInfoLabelColleges.setText(f"Page {self.colleges_page} of {self.colleges_total_pages}")
    ...
    def sort_programs_by_dropdown(self, sort_by):
        sort_mapping = {'Program': 'name', 'College': 'college'}
        key = sort_mapping.get(sort_by)
        if key:
    ...
    def load_programs_table(self, refresh=True):
        ...
        filtered = [
            ...
            if not search_text
            or search_text in p['name'].lower()
            or search_text in p['code'].lower()
            or search_text in (p.get('college') or '').lower()
        ]
        ...
        for row, program in enumerate(programs):
            ...
            table.setItem(row, 2, QTableWidgetItem(program.get('college') or ''))
    ...
    def add_action_buttons_program(self, row, program):
        widget = self._make_action_buttons(
            lambda checked, p=program: self.edit_program(p),
            lambda checked, p=program: self.delete_program(p)
        )
        self.dataTablePrograms.setCellWidget(row, 3, widget)
    ...
    def delete_program(self, program):
        try:
            ...
            affected = [s for s in students if s['program_code'] == program['code']]
            ...
            if affected:
                ...
                if len(affected) > 5:
                    student_list += f"\n  • ...and {len(affected) - 5} more"
                warning_msg += (
                    f"\n\n⚠️  WARNING: This program has {len(affected)} enrolled student(s):\n{student_list}"
                    f"\n\nThese students will have their program set to NULL."
                )
            warning_msg += "\n\nDo you want to continue?"
    ...
    def update_programs_pagination(self):
        self.pageInfoLabelPrograms.setText(f"Page {self.programs_page} of {self.programs_total_pages}")
